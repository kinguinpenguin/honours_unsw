...
\[\log \frac{q(y | y^*)}{q(y^* | y)} = \log \frac{1}{0.5} = -\log \frac{1}{2}.\] For any other edge-state, the move probabilities are symmetric, so the proposal log-ratio is \[\log \frac{q(y | y^*)}{q(y^* | y)} = \log \frac{0.5}{0.5} = 0.\] That is, the log acceptance probability changes when an alter is ranked either on or next to the maximum or minimum by an ego.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
