...
\chapter{Optimising the Infrastructure of \texttt{ergm.rank}}\label{cha}
Our main goal is to be able to, given an observed ranking, we want a statistical model that explains this ranking, as well as being able to simulate changes that could have happened to the rankings, as well as simulate new rankings being added.
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
We accept the proposal if for $U \sim \text{Unif}(0, 1)$ we have $U \leq \alpha \Leftrightarrow \log U \leq \log \alpha.$ Note that $q(y^* | y)$ represents the probability of going from state $y$ to state $y^*$, and vice-versa for $q(y | y^*)$. Now, consider the edge-state for an altar rated the maximum or minimum by an ego ($n$ and $1$ respectively). Then, the probability for $q(y | y^*)$ and $q(y^* | y)$ would not be the same, as for a proposed state $y^* = \text{max} - 1$, the probability of $q(y^* | y) = 1$, as it is not possible to go up at the maximum, whereas the probability of $q(y | y^*) = 0.5$ since at $y^* = \text{max} - 1$ the edge-state can move either up or down by 1. This is similarly the case for the minimum $y = 1$, and so in both cases we have that $\log \frac{q(y | y^*)}{q(y^* | y)} = \log \frac{0.5}{1} = \log \frac{1}{2}$.
\hfill \break \hfill \break
Similarly, for the edge-states where an altar is rated one above the minimum or one below the maximum, the probability would be flipped, and we'd instead have
$\log \frac{q(y | y^*)}{q(y^* | y)} = \log \frac{1}{0.5} = -\log \frac{1}{2}.$ For any other edge-state, the probability of moving from $y$ to the proposed $y^*$ would be equal, and so it would have log-ratio $\log 1 = 0.$ That is, the acceptance probability changes when an alter is ranked either on or next to the maximum or minimum by an ego.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
