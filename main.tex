...
\end{figure}
\hfill \break
For example, for an ego $i$ and an alter $j$, $\texttt{udsm[i][j] = k}$ means that the alter $k$ is the individual ranked immediately above $j$ by $i$. Then, it becomes more cost efficient to iterate over the network, since in most cases it would only be necessary to iterate between two rankings when recalculating statistics, as suggested by the formulas in Chapter 2. Excluding ties, on average it would be expected to cut computational time by two-thirds (i think ask pavel tomorrow), because (reason ask him).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
