...
The main goal of this thesis is to be able to, given an observed ranking, we want a statistical model that explains this ranking, as well as being able to simulate changes that could have happened to the rankings, as well as simulate new rankings being added. Ideally, we aim to simulate networks with up to 100 individuals. However, this proves to be a problem in time complexity, as the functions which simulate changes to the network for deference and local or global nonconformity, namely $\texttt{c\_deference}$, $\texttt{c\_nonconformity}$ and $\texttt{c\_localAND\_nonconformity}$ respectively. There are also some other functions which were changed, but these functions were the main offenders for simulation, each with a time complexity of $\mathcal{O}(n^3).$
\hfill \break \hfill \break
...
\caption{Consider an ego $i$ and their rankings of alters $a, b$ and $c.$ Then, if \texttt{sm[i][a]} were changed, using \texttt{udsm} we can iterate either up or down to the new value of \texttt{sm[i][a]}, allowing us to easily recalculate statistics such as nonconformity without iterating over individuals who were not affected by the change.}
\end{figure}
\hfill \break
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
