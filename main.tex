...
Inconsistency in a given network is a measure of the tendency of each ego to rank alters discordantly. That is, whether an ego follows a trend to their alter rankings, as we expect that an ego will tend to exhibit consistency in alter ratings \citep{krivitsky2017exponential}. For two ranking structures $\boldsymbol{y}$ and $\boldsymbol{y} \prime$ on vertex sets $N$ and $N^{\prime}$ such that $N_s$ represents the actors involved in both networks, the statistic measuring inconsistency is given by
...
Our main goal is to be able to, given an observed ranking, we want a statistical
model that explains this ranking, as well as being able to simulate changes that could
have happened to the rankings, as well as simulate new rankings being added. 
\section{Dealing with
Computational Time Issues}
Ideally, we aim to simulate networks with up to 100 individuals. However, this proves to be a problem in time complexity, as the functions which simulate changes to the network for deference and local or global nonconformity, namely $\texttt{c\_deference}$, $\texttt{c\_nonconformity}$ and $\texttt{c\_localAND\_nonconformity}$ respectively. There are also some other functions which were changed, but these functions were the main offenders for simulation, each with a time complexity of $\mathcal{O}(n^3).$
\hfill \break \hfill \break
...
\end{figure}
\hfill \break
For example, for an ego $i$ and an alter $j$, $\texttt{udsm[i][j] = k}$ means that the alter $k$ is the individual ranked immediately above $j$ by $i$. Then, it becomes more cost efficient to iterate over the network, since it would only be necessary to iterate between two rankings when recalculating statistics, as evident in the formulas from Chapter 2. 
\hfill \break \hfill \break
Suppose we arbitrarily choose 2 points on a line uniformly. On average, the distance between these points would be a third of the length of the line. As such, it's expected that iterating only between the old value which $i$ ranked $j$ and the new value that $i$ ranked $j$, the time complexity would be cut by two-thirds.
\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{Figures/udsm.pdf}
\caption{Consider an ego $i$ and their rankings of alters $a, b$ and $c.$ Then, if \texttt{sm[i][a]} were changed, using \texttt{udsm} we can start at the original value for \texttt{sm[i][a]} and iterate either up or down to the new value of \texttt{sm[i][a]}, allowing us to easily recalculate statistics such as nonconformity without iterating over individuals who were not affected by the change.}
\end{figure}
\hfill \break
...
Using the new auxiliary structure, we apply the framework to larger networks to benchmark computation time. We also evaluate whether the redesigned proposal mechanism produces accurate MLE estimates on partially observed networks, compared to MLE estimates obtained from the full network.
...
The benchmarking process takes the Newcomb dataset and generates a modified version containing missing data. In the first benchmark script, all rankings outside of the top five were replaced with NA to imitate a missing dataset. Two models are then fitted for each of the fifteen weeks in the dataset:
\hfill \break
\begin{itemize}
...
\end{itemize}
\hfill \break
For the missing data model, it is assumed that there exists a complete ordering beneath the observed partial ordering, so that the adjacent alter swap proposal can be used. The fitted MLEs for each week are stored in an indexed list.
\hfill \break
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Benchmarks/coef_trajectories_deference.png}
\caption{Parameter estimates over time for deference, where "constrained" and "true" refer to the top-5 data and complete data MLE respectively, with shaded regions indicating $\pm$1 standard error.}
\end{figure}
\hfill \break
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Benchmarks/coef_trajectories_nonconformity.png}
\caption{Parameter estimates over time for global nonconformity, where the constrained model takes only the top 5 rankings of each ego.}
\end{figure}
\hfill \break \hfill \break
...
\hfill \break \hfill \break
Comparatively, for the fitted MLE using only the top-five rankings, the parameter estimates for global nonconformity show similar values to the true model, but trend slightly below it as the weeks progress. This suggests that in the early weeks, there was limited agreement among individuals regarding whom they ranked in their top five, which follows as social circles and friendships had not yet been formed. However, as time went on, individuals appeared to reach more of an agreement about whom they considered their top five friends compared to others in the group. Very little information was lost by restricting the data to the top-five rankings, even when compared to the deference parameter, as the estimates between the two models lie within half a standard error of each other for a majority of the weeks. Interestingly, when we compare this instead to a fitted MLE using a dataset removing only the top 5 rankings:
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Benchmarks/coef_trajectories_nonconformity_nottop5.png}
\caption{Parameter estimates over time for global nonconformity, where the constrained model takes the rankings outside of the top 5 rankings of each ego.}
\end{figure}
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{Benchmarks/coef_trajectories_nonconformity.localAND.png}
\caption{Parameter estimates over time for the global and local nonconformity.}
\end{figure}
\hfill \break \hfill \break
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\hfill \break \hfill \break
Comparatively, for the fitted MLE taking only the top-5 rankings, we see that the parameter estimates for global nonconformity start at approxmiately -0.015, and trend towards 0 as weeks passed. This implies that within the first few weeks, there may have been a stronger degree of agreement among participants, regarding who the most highly ranked individuals were within the fraternity network. However, as the weeks progressed, this agreement diminished as participants formed friendships and relationships within the group, resulting in a greater divergence in who they regarded as their top five friends.
\clearpage
